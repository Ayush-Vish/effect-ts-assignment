...
describe("API Endpoints", () => {
  ...
  it("should create a new task for the specified user", async () => {
    ...
    expect(res.body.data).toHaveProperty("name", taskData.name);
    expect(res.body.data).toHaveProperty("status", taskData.status);
  ...
  it("should retrieve all tasks for the specified user", async () => {
    const res = await request(server).get(`/users/${userId}/tasks`);
    expect(res.statusCode).toEqual(200);
  ...
  it("should retrieve a specific task for the specified user", async () => {
    const res = await request(server).get(`/users/${userId}/tasks/${taskId}`);
    expect(res.statusCode).toEqual(200);
